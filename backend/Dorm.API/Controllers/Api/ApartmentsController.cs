    ...
    ICurrentUser currentUser) : ControllerBase
{
    ...
    /// <summary>Returns whether the owner needs to pay a listing fee before creating a new apartment.</summary>
    [HttpGet("requires-listing-fee")]
    [Authorize(Policy = AuthPolicies.Owner)]
    public async Task<ActionResult<object>> RequiresListingFee(CancellationToken ct)
    {
        var ownerId = currentUser.UserId ?? throw new UnauthorizedException();
        var required = await apartments.RequiresListingFeeAsync(ownerId, ct);
        return Ok(new { required, amount = required ? 10m : 0m });
    }
